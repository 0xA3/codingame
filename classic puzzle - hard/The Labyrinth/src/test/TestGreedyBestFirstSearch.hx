...
@:access( Main )
class TestGreedyBestFirstSearch extends buddy.BuddySuite {
	
	public function new() {
		
		describe( "Test GreedyBestFirstSearch", {
			...
			it( "3-node Graph", {
				...
				final path = GreedyBestFirstSearch.getPath( nodes, p.s, p.g );
			...
			it( "Small Graph A", {
				...
				final nodes = Main.createNodes( p.n, p.distances, p.edges );
				final path = GreedyBestFirstSearch.getPath( nodes, p.s, p.g );
				path.toString().should.be( "[0,2,3,4]" );
			});
			...
			it( "Slightly Larger Graph", {
				...
				final nodes = Main.createNodes( p.n, p.distances, p.edges );
				final path = GreedyBestFirstSearch.getPath( nodes, p.s, p.g );
				path.toString().should.be( "[0,3,4,6,7]" );
			});
