...
class Gold12 extends Agent2 {
	...
	function attack() {
		...
		final controlMobs = mobs.filter( mob -> {
			final isInHeroRange = mob.position.distanceSq( me.heros[ATTACKER].position ) <= Config.HERO_VIEW_RADIUS * Config.HERO_VIEW_RADIUS;
			return 	mob.shieldDuration == 0 &&
					mob.threatFor == 0 &&
					isInHeroRange;
		...
		} else if( me.mana >= Config.SPELL_CONTROL_COST && controlMobs.length > 0 ) {
			final controlMob = controlMobs[0];
			control( ATTACKER, controlMob.id, opp.basePosition, 'control ${controlMob.id}' );
			// trace( '$turn control ${controlMob.id}' );
