...
@:access(ooc.OpponentManager)
class TestOpponentManager extends buddy.BuddySuite {
	...
	public function new() {
		...
		describe( "Test Move", {
			...
			map.init();
			var opponentManager:ooc.OpponentManager;

			beforeEach({
				opponentManager = new ooc.OpponentManager( width, height, map );
