...
@:access(MainBFS)
class TestProcessDFS extends buddy.BuddySuite{
	...
	public function new() {
		...
		describe( "Test process", {
			it( "Basic North", {
				final ip = basicNorth;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "3 leagues" );
			});
			it( "Basic East", {
				final ip = basicEast;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "7 leagues" );
			});
			it( "Basic South", {
				final ip = basicSouth;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "2 leagues" );
			});
			it( "Basic West", {
				final ip = basicWest;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "6 leagues" );
			});
			it( "Basic North-East", {
				final ip = basicNorthEast;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "4 leagues" );
			});
			it( "Basic South-East", {
			...
			it( "North and NE (Open)", {
				...
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "5 leagues" );
			});
			it( "East and SE (Open)", {
				final ip = eastAndSeOpen;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "13 leagues" );
			});
			...
			it( "West and NW (Open)", {
				...
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "10 leagues" );
			});
			it( "Simple Horizontal Journey", {
				final ip = simpleHorizontalJourney;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "1 league" );
			});
			...
			it( "Around the World 1", {
				...
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "18 leagues" );
			});
			...
			it( "Maze 1", {
				final ip = maze_1;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "34 leagues" );
			});
			it( "Maze 2", {
				final ip = maze_2;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "24 leagues" );
			});
			it( "Large Maze", {
				final ip = largeMaze;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "157 leagues" );
			});
			it( "Simple Outside Traversal", {
				final ip = simpleOutsideTraversal;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "23 leagues" );
			});
			it( "Horizontal Outside Traversal", {
				final ip = horizontalOutsideTraversal;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "43 leagues" );
			});
			it( "Vertical Outside Traversal", {
				final ip = verticalOutsideTraversal;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "22 leagues" );
			});
			it( "Complete Traversal", {
				final ip =completeTraversal;
				MainBFS.process( ip.w, ip.h, ip.grid ).should.be( "59 leagues" );
			});
		});
	}
