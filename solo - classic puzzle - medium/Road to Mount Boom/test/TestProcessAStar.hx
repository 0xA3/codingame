...
@:access(MainAStar)
class TestProcessAStar extends buddy.BuddySuite{
	...
	public function new() {
		...
		describe( "Test process", {
			it( "Basic North", {
				final ip = basicNorth;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "3 leagues" );
			});
			it( "Basic East", {
				final ip = basicEast;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "7 leagues" );
			});
			it( "Basic South", {
				final ip = basicSouth;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "2 leagues" );
			});
			it( "Basic West", {
				final ip = basicWest;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "6 leagues" );
			});
			it( "Basic North-East", {
				final ip = basicNorthEast;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "4 leagues" );
			});
			it( "Basic South-East", {
			...
			it( "North and NE (Open)", {
				...
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "5 leagues" );
			});
			it( "East and SE (Open)", {
				final ip = eastAndSeOpen;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "13 leagues" );
			});
			...
			it( "West and NW (Open)", {
				...
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "10 leagues" );
			});
			it( "Simple Horizontal Journey", {
				final ip = simpleHorizontalJourney;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "1 league" );
			});
			...
			it( "Maze 1", {
				final ip = maze_1;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "34 leagues" );
			});
			it( "Maze 2", {
				final ip = maze_2;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "24 leagues" );
			});
			it( "Large Maze", {
				final ip = largeMaze;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "157 leagues" );
			});
			it( "Simple Outside Traversal", {
				final ip = simpleOutsideTraversal;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "23 leagues" );
			});
			it( "Horizontal Outside Traversal", {
				final ip = horizontalOutsideTraversal;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "43 leagues" );
			});
			it( "Vertical Outside Traversal", {
				final ip = verticalOutsideTraversal;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "22 leagues" );
			});
			it( "Complete Traversal", {
				final ip =completeTraversal;
				MainAStar.process( ip.w, ip.h, ip.grid ).should.be( "59 leagues" );
			});
		});
	}
